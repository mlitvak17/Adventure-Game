# Project Title: (Adventure Game (Non Linear)) "ESCAPE ROOM"
# Creators: Pierce Chaffin and Mauricio Litvak and Shaun Doucherty
# Created for CMPSC 121 Spring 2017 PSU UP

print('This game was made for CMPSC 121 Spring 2017')
print('Coded by Mauricio Litvak and Pierce Chaffin')
print('This game will be highly unsatisfying :)')
print('...Game starts in...')
for i in range(6,0,-1):
    print(i)
name = input('Enter your name: ')
print()
print()
print()
print()
for i in range(4):
    print('Drip...')
print('*A drop of water rolls down your face and you wake with a dull pain in your head*')
wake_count = 0
while 0 == 0:
    wake_condition = input('Wake up? *yes or no*\n')
    if wake_condition == 'yes':
        wake_count += 1
        if wake_count == 3:
            break
        else:
            print('*you pass out again*')
            continue
    else:
        print('You fall into deep sleep again\n *hours pass*')
        continue
print('*you finally awake to complete darkness*')
while 0==0:
    option1 = input('*Try to get up or take in your surroundings* (Enter a or b)\n')
    if option1 == 'a':
        print('*You try to get up but upon getting up you face severe pain in your leg and realize that its completely broken and pass out in pain*')
        print()
        print()
        print()
        print('*you awake a little later*')
        break
    else:
        print('You attempt to take in your surroundings but you are stopped by you cannot due to the insurmountable pain in your leg.')
print()
print()
print()
print('What do you do next, do you...')
print('1. Check your pockets for their contents')
print('2. Attempt to stand up.')
print('3. Go back to sleep')
while 0 == 0:
    n = int(input('Enter 1, 2, or 3\n'))
    if n == 1:
        print('You check your pockets and find a lighter, and a crumpled a piece of paper.')
        break
    elif n == 2:
        print('You attempt to stand up but to no avail.')
    elif n == 3:
        print('You go back to sleep.')
    else:
        print('You did not enter an integer between 1 and 3.')
if (n == 1) or (n == 2) or (n == 3):
    print('you unroll the paper to find a simple phrase scribbled on it in what seems to be ink, in the light of your lighter\n*Trust the Process*')
    print('Upon reading this you hear an ubrupt noise in the nearby corner of the room and see a sliver of light gleaming through what seems to be a now cracked open door way')
    print('You see a faint shadow through the crack vanish into what waits outside')
    print('Do you go outside?')
    decision = input('y or n\n')
    if (decision == 'y') or (decision == 'n'):
        print('you crawl over to the door and peer outside')
        print('outside lies a hallway, dimly lit')
        print('you pry the door back till its fully open and then take it all in')
        print('you find there to be a spiked bat on the floor along side painkillers with a note beside them, reading "xFor your leg %s"' % name)
        print('do you pick up both or just the painkillers?')
        decision = input('a or b\n')
        if decision == 'a':
            print('you pick up the painkillers and down a few and sit around for a little to let them kick in')
            print('the loudspeaker kicks on and in a scratchy voice you hear "a fighter...interesting"')
            for i in range(20):
                print('.')
            print('You feel the painkillers set in and now try to stand')
            print('With a little wince of pain you get up reluctantly but alas can stand')
            print('You grab the bat on the ground and look onward towards the hallway')
            print('do you go down the hallway towards the staircase or turn left to explore the floor')
            decision = input('a or b\n')
            if decision == 'a':
                print('you start to tread down the hallway, firmly grasping the bat')
                print('as you continue down the hall, you hear a buzz on what seems to be a loudspeaker followed by a slight noise rather similar to breathing, followed by the same silence that accompanied you before')
                print('you hear a sharp fizzing noise followed by a pop')
                print('The hallway lights fizz out and you are left to the pitch black of before')
                print('Do you pull out your lighter or do you trust you remember where the stair door was')
                decision = input('a or b\n')
                if decision == 'a':
                    print('You reach for your lighter but feel nothing left in your pocket but the scrap of paper with\n*Trust the process*\nscrawled on it')
                    print('The all but familiar static of the loud speaker is heard again')
                    print('The voice speaks again saying in a rather raspy older bodied voice \n"Ahhhhh Do you not Trust %s?"' % name)
                    print('You stand in the darkness trembling, feeling light headed, with pain bolting down your leg with every step you do and dont take')
                    print('Do you respond to the voice?')
                    decsion = input('a or b\n')
                    if decision == 'a':
                        print('What do you say?')
                        speech = input()
                        print('%s...interesting...you must trust %s, its the only way' % (speech, name))
                        print('you scream "WHAT DO YOU MEAN???"')
                        print('the loudspeaker fizzes out and the voice never responds')
                        print('you decide to sit down a regroup')
                        for i in range(40):
                            print('.')
                        print('You look up and notice that two strips of purple light are turning on dully on the ceiling')
                        print('Upon futher evaluation you notice the walls are slightly glowing but not fully, only in some sort of distinct shape')
                        print('They seem to resemble a path with the phrase \n*Trust*\n scrawled on the wall in blood and the logo of a corporation that looks somewhat familiar')
                        print('You get up and decide to follow the path on the wall and it leads you to the stairway door')
                        print('You open the door and find not a staricase but an elongated roo, with what seems to be a figure sitting down, staring out a window')
                        print('You move closer into the room and pause for a second')
                        print('A great metal clap is heard from behind you and you find that the door that you entered through is no longer existant')
                        print('Do you approach the figure or call to them at a distance?')
                        decision = input('a or b\n')
                        if decision == 'a':
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                        else:
                            print('You take a few more steps forward and call out')
                            print('Hello? Who are you?')
                            print('The loudspeaker static is heard again')
                            print('DO YOU REALLY THINK IT WOULD BE THAT EASY')
                            print('in a flash of static and light you find that the chair along with the individual has vanished')
                            print('But you hear breathing coming from behind you and turn to inspect...')
                            print('The individual in the chair now resides before you')
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                    else:
                        print('the loudspeaker fizzes out')
                        print('you decide to sit down a regroup')
                        for i in range(40):
                            print('.')
                        print('You look up and notice that two strips of purple light are turning on dully on the ceiling')
                        print('Upon futher evaluation you notice the walls are slightly glowing but not fully, only in some sort of distinct shape')
                        print('They seem to resemble a path with the phrase \n*Trust*\n scrawled on the wall in blood and the logo of a corporation that looks somewhat familiar')
                        print('You get up and decide to follow the path on the wall and it leads you to the stairway door')
                        print('You open the door and find not a staricase but an elongated roo, with what seems to be a figure sitting down, staring out a window')
                        print('You move closer into the room and pause for a second')
                        print('A great metal clap is heard from behind you and you find that the door that you entered through is no longer existant')
                        print('Do you approach the figure or call to them at a distance?')
                        decision = input('a or b\n')
                        if decision == 'a':
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                        else:
                            print('You take a few more steps forward and call out')
                            print('Hello? Who are you?')
                            print('The loudspeaker static is heard again')
                            print('DO YOU REALLY THINK IT WOULD BE THAT EASY')
                            print('in a flash of static and light you find that the chair along with the individual has vanished')
                            print('But you hear breathing coming from behind you and turn to inspect...')
                            print('The individual in the chair now resides before you')
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                else:
                    print('You stumble and reach for the wall for some guidance')
                    print('The speaker static is heard again')
                    print('NOT ONLY DO WE HAVE A FIGHTER...WE HAVE AN IGNORANT, ARROGANT FIGHTER IN OUR MIDST...ILL HELP YOU OUT.....')
                    if 0 == 0:
                        print('the loudspeaker fizzes out and the voice never responds')
                        print('you decide to sit down a regroup')
                        for i in range(40):
                            print('.')
                        print('You look up and notice that two strips of purple light are turning on dully on the ceiling')
                        print('Upon futher evaluation you notice the walls are slightly glowing but not fully, only in some sort of distinct shape')
                        print('They seem to resemble a path with the phrase \n*Trust*\n scrawled on the wall in blood and the logo of a corporation that looks somewhat familiar')
                        print('You decide to follow the path on the wall and it leads you to the stairway door')
                        print('You open the door and find not a staricase but an elongated room=, with what seems to be a figure sitting down, staring out a window')
                        print('You move closer into the room and pause for a second')
                        print('A great metal clap is heard from behind you and you find that the door that you entered through is no longer existant')
                        print('Do you approach the figure or call to them at a distance?')
                        decision = input('a or b\n')
                        if decision == 'a':
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                        else:
                            print('You take a few more steps forward and call out')
                            print('Hello? Who are you?')
                            print('The loudspeaker static is heard again')
                            print('DO YOU REALLY THINK IT WOULD BE THAT EASY')
                            print('in a flash of static and light you find that the chair along with the individual has vanished')
                            print('But you hear breathing coming from behind you and turn to inspect...')
                            print('The individual in the chair now resides before you')
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                    else:
                        print('the loudspeaker fizzes out')
                        print('you decide to sit down a regroup')
                        for i in range(40):
                            print('.')
                        print('You look up and notice that two strips of purple light are turning on dully on the ceiling')
                        print('Upon futher evaluation you notice the walls are slightly glowing but not fully, only in some sort of distinct shape')
                        print('They seem to resemble a path with the phrase \n*Trust*\n scrawled on the wall in blood and the logo of a corporation that looks somewhat familiar')
                        print('You get up and decide to follow the path on the wall and it leads you to the stairway door')
                        print('You open the door and find not a staricase but an elongated roo, with what seems to be a figure sitting down, staring out a window')
                        print('You move closer into the room and pause for a second')
                        print('A great metal clap is heard from behind you and you find that the door that you entered through is no longer existant')
                        print('Do you approach the figure or call to them at a distance?')
                        decision = input('a or b\n')
                        if decision == 'a':
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                        else:
                            print('You take a few more steps forward and call out')
                            print('Hello? Who are you?')
                            print('The loudspeaker static is heard again')
                            print('DO YOU REALLY THINK IT WOULD BE THAT EASY')
                            print('in a flash of static and light you find that the chair along with the individual has vanished')
                            print('But you hear breathing coming from behind you and turn to inspect...')
                            print('The individual in the chair now resides before you')
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
            else:
                print('You start to venture down the other hallway')
                print('The speaker sizzles to life')
                print('TRUST THE PROCESS...FOLLOW THE PATH')
                print('and the static is heard again...')
                print('you start to tread down the correct hallway, firmly grasping the bat')
                print('as you continue down the hall, you hear a buzz on what seems to be a loudspeaker followed by a slight noise rather similar to breathing, followed by the same silence that accompanied you before')
                print('you hear a sharp fizzing noise followed by a pop')
                print('The hallway lights fizz out and you are left to the pitch black of before')
                print('Do you pull out your lighter or do you trust you remember where the stair door was')
                decision = input('a or b\n')
                if decision == 'a':
                    print('You reach for your lighter but feel nothing left in your pocket but the scrap of paper with\n*Trust the process*\nscrawled on it')
                    print('The all but familiar static of the loud speaker is heard again')
                    print('The voice speaks again saying in a rather raspy older bodied voice \n"Ahhhhh Do you not Trust %s?"' % name)
                    print('You stand in the darkness trembling, feeling light headed, with pain bolting down your leg with every step you do and dont take')
                    print('Do you respond to the voice?')
                    decsion = input('a or b\n')
                    if decision == 'a':
                        print('What do you say?')
                        speech = input()
                        print('%s...interesting...you must trust %s, its the only way' % (speech, name))
                        print('you scream "WHAT DO YOU MEAN???"')
                        print('the loudspeaker fizzes out and the voice never responds')
                        print('you decide to sit down a regroup')
                        for i in range(40):
                            print('.')
                        print('You look up and notice that two strips of purple light are turning on dully on the ceiling')
                        print('Upon futher evaluation you notice the walls are slightly glowing but not fully, only in some sort of distinct shape')
                        print('They seem to resemble a path with the phrase \n*Trust*\n scrawled on the wall in blood and the logo of a corporation that looks somewhat familiar')
                        print('You get up and decide to follow the path on the wall and it leads you to the stairway door')
                        print('You open the door and find not a staricase but an elongated roo, with what seems to be a figure sitting down, staring out a window')
                        print('You move closer into the room and pause for a second')
                        print('A great metal clap is heard from behind you and you find that the door that you entered through is no longer existant')
                        print('Do you approach the figure or call to them at a distance?')
                        decision = input('a or b\n')
                        if decision == 'a':
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                        else:
                            print('You take a few more steps forward and call out')
                            print('Hello? Who are you?')
                            print('The loudspeaker static is heard again')
                            print('DO YOU REALLY THINK IT WOULD BE THAT EASY')
                            print('in a flash of static and light you find that the chair along with the individual has vanished')
                            print('But you hear breathing coming from behind you and turn to inspect...')
                            print('The individual in the chair now resides before you')
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                    else:
                        print('the loudspeaker fizzes out')
                        print('you decide to sit down a regroup')
                        for i in range(40):
                            print('.')
                        print('You look up and notice that two strips of purple light are turning on dully on the ceiling')
                        print('Upon futher evaluation you notice the walls are slightly glowing but not fully, only in some sort of distinct shape')
                        print('They seem to resemble a path with the phrase \n*Trust*\n scrawled on the wall in blood and the logo of a corporation that looks somewhat familiar')
                        print('You get up and decide to follow the path on the wall and it leads you to the stairway door')
                        print('You open the door and find not a staricase but an elongated roo, with what seems to be a figure sitting down, staring out a window')
                        print('You move closer into the room and pause for a second')
                        print('A great metal clap is heard from behind you and you find that the door that you entered through is no longer existant')
                        print('Do you approach the figure or call to them at a distance?')
                        decision = input('a or b\n')
                        if decision == 'a':
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                        else:
                            print('You take a few more steps forward and call out')
                            print('Hello? Who are you?')
                            print('The loudspeaker static is heard again')
                            print('DO YOU REALLY THINK IT WOULD BE THAT EASY')
                            print('in a flash of static and light you find that the chair along with the individual has vanished')
                            print('But you hear breathing coming from behind you and turn to inspect...')
                            print('The individual in the chair now resides before you')
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                else:
                    print('You stumble and reach for the wall for some guidance')
                    print('The speaker static is heard again')
                    print('NOT ONLY DO WE HAVE A FIGHTER...WE HAVE AN IGNORANT, ARROGANT FIGHTER IN OUR MIDST...ILL HELP YOU OUT.....')
                    if 0 == 0:
                        print('the loudspeaker fizzes out and the voice never responds')
                        print('you decide to sit down a regroup')
                        for i in range(40):
                            print('.')
                        print('You look up and notice that two strips of purple light are turning on dully on the ceiling')
                        print('Upon futher evaluation you notice the walls are slightly glowing but not fully, only in some sort of distinct shape')
                        print('They seem to resemble a path with the phrase \n*Trust*\n scrawled on the wall in blood and the logo of a corporation that looks somewhat familiar')
                        print('You decide to follow the path on the wall and it leads you to the stairway door')
                        print('You open the door and find not a staricase but an elongated room=, with what seems to be a figure sitting down, staring out a window')
                        print('You move closer into the room and pause for a second')
                        print('A great metal clap is heard from behind you and you find that the door that you entered through is no longer existant')
                        print('Do you approach the figure or call to them at a distance?')
                        decision = input('a or b\n')
                        if decision == 'a':
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                        else:
                            print('You take a few more steps forward and call out')
                            print('Hello? Who are you?')
                            print('The loudspeaker static is heard again')
                            print('DO YOU REALLY THINK IT WOULD BE THAT EASY')
                            print('in a flash of static and light you find that the chair along with the individual has vanished')
                            print('But you hear breathing coming from behind you and turn to inspect...')
                            print('The individual in the chair now resides before you')
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                    else:
                        print('the loudspeaker fizzes out')
                        print('you decide to sit down a regroup')
                        for i in range(40):
                            print('.')
                        print('You look up and notice that two strips of purple light are turning on dully on the ceiling')
                        print('Upon futher evaluation you notice the walls are slightly glowing but not fully, only in some sort of distinct shape')
                        print('They seem to resemble a path with the phrase \n*Trust*\n scrawled on the wall in blood and the logo of a corporation that looks somewhat familiar')
                        print('You get up and decide to follow the path on the wall and it leads you to the stairway door')
                        print('You open the door and find not a staricase but an elongated roo, with what seems to be a figure sitting down, staring out a window')
                        print('You move closer into the room and pause for a second')
                        print('A great metal clap is heard from behind you and you find that the door that you entered through is no longer existant')
                        print('Do you approach the figure or call to them at a distance?')
                        decision = input('a or b\n')
                        if decision == 'a':
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                        else:
                            print('You take a few more steps forward and call out')
                            print('Hello? Who are you?')
                            print('The loudspeaker static is heard again')
                            print('DO YOU REALLY THINK IT WOULD BE THAT EASY')
                            print('in a flash of static and light you find that the chair along with the individual has vanished')
                            print('But you hear breathing coming from behind you and turn to inspect...')
                            print('The individual in the chair now resides before you')
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

        else:
            print('the loudspeaker comes to life quickly')
            print('PICK UP THE BAT, FIGHTER')
            print('you pick up the painkillers and bat and down a few and sit around for a little to let them kick in')
            for i in range(20):
                print('.')
            print('You feel the painkillers set in and now try to stand')
            print('With a little wince of pain you get up reluctantly but alas can stand')
            print('You grab the bat on the ground and look onward towards the hallway')
            print('do you go down the hallway towards the staircase or turn left to explore the floor')
            decision = input('a or b\n')
            if decision == 'a' or decision == 'b':
                print('you start to tread down the hallway, firmly grasping the bat')
                print('as you continue down the hall, you hear a buzz on what seems to be a loudspeaker followed by a slight noise rather similar to breathing, followed by the same silence that accompanied you before')
                print('you hear a sharp fizzing noise followed by a pop')
                print('The hallway lights fizz out and you are left to the pitch black of before')
                print('Do you pull out your lighter or do you trust you remember where the stair door was')
                decision = input('a or b\n')
                if decision == 'a':
                    print('You reach for your lighter but feel nothing left in your pocket but the scrap of paper with\n*Trust the process*\nscrawled on it')
                    print('The all but familiar static of the loud speaker is heard again')
                    print('The voice speaks again saying in a rather raspy older bodied voice \n"Ahhhhh Do you not Trust %s?"' % name)
                    print('You stand in the darkness trembling, feeling light headed, with pain bolting down your leg with every step you do and dont take')
                    print('Do you respond to the voice?')
                    decsion = input('a or b\n')
                    if decision == 'a':
                        print('What do you say?')
                        speech = input()
                        print('%s...interesting...you must trust %s, its the only way' % (speech, name))
                        print('you scream "WHAT DO YOU MEAN???"')
                        print('the loudspeaker fizzes out and the voice never responds')
                        print('you decide to sit down a regroup')
                        for i in range(40):
                            print('.')
                        print('You look up and notice that two strips of purple light are turning on dully on the ceiling')
                        print('Upon futher evaluation you notice the walls are slightly glowing but not fully, only in some sort of distinct shape')
                        print('They seem to resemble a path with the phrase \n*Trust*\n scrawled on the wall in blood and the logo of a corporation that looks somewhat familiar')
                        print('You get up and decide to follow the path on the wall and it leads you to the stairway door')
                        print('You open the door and find not a staricase but an elongated roo, with what seems to be a figure sitting down, staring out a window')
                        print('You move closer into the room and pause for a second')
                        print('A great metal clap is heard from behind you and you find that the door that you entered through is no longer existant')
                        print('Do you approach the figure or call to them at a distance?')
                        decision = input('a or b\n')
                        if decision == 'a':
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                        else:
                            print('You take a few more steps forward and call out')
                            print('Hello? Who are you?')
                            print('The loudspeaker static is heard again')
                            print('DO YOU REALLY THINK IT WOULD BE THAT EASY')
                            print('in a flash of static and light you find that the chair along with the individual has vanished')
                            print('But you hear breathing coming from behind you and turn to inspect...')
                            print('The individual in the chair now resides before you')
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                    else:
                        print('the loudspeaker fizzes out')
                        print('you decide to sit down a regroup')
                        for i in range(40):
                            print('.')
                        print('You look up and notice that two strips of purple light are turning on dully on the ceiling')
                        print('Upon futher evaluation you notice the walls are slightly glowing but not fully, only in some sort of distinct shape')
                        print('They seem to resemble a path with the phrase \n*Trust*\n scrawled on the wall in blood and the logo of a corporation that looks somewhat familiar')
                        print('You get up and decide to follow the path on the wall and it leads you to the stairway door')
                        print('You open the door and find not a staricase but an elongated roo, with what seems to be a figure sitting down, staring out a window')
                        print('You move closer into the room and pause for a second')
                        print('A great metal clap is heard from behind you and you find that the door that you entered through is no longer existant')
                        print('Do you approach the figure or call to them at a distance?')
                        decision = input('a or b\n')
                        if decision == 'a':
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                        else:
                            print('You take a few more steps forward and call out')
                            print('Hello? Who are you?')
                            print('The loudspeaker static is heard again')
                            print('DO YOU REALLY THINK IT WOULD BE THAT EASY')
                            print('in a flash of static and light you find that the chair along with the individual has vanished')
                            print('But you hear breathing coming from behind you and turn to inspect...')
                            print('The individual in the chair now resides before you')
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                else:
                    print('You stumble and reach for the wall for some guidance')
                    print('The speaker static is heard again')
                    print('NOT ONLY DO WE HAVE A FIGHTER...WE HAVE AN IGNORANT, ARROGANT FIGHTER IN OUR MIDST...ILL HELP YOU OUT.....')
                    if 0 == 0:
                        print('the loudspeaker fizzes out and the voice never responds')
                        print('you decide to sit down a regroup')
                        for i in range(40):
                            print('.')
                        print('You look up and notice that two strips of purple light are turning on dully on the ceiling')
                        print('Upon futher evaluation you notice the walls are slightly glowing but not fully, only in some sort of distinct shape')
                        print('They seem to resemble a path with the phrase \n*Trust*\n scrawled on the wall in blood and the logo of a corporation that looks somewhat familiar')
                        print('You decide to follow the path on the wall and it leads you to the stairway door')
                        print('You open the door and find not a staricase but an elongated room=, with what seems to be a figure sitting down, staring out a window')
                        print('You move closer into the room and pause for a second')
                        print('A great metal clap is heard from behind you and you find that the door that you entered through is no longer existant')
                        print('Do you approach the figure or call to them at a distance?')
                        decision = input('a or b\n')
                        if decision == 'a':
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                        else:
                            print('You take a few more steps forward and call out')
                            print('Hello? Who are you?')
                            print('The loudspeaker static is heard again')
                            print('DO YOU REALLY THINK IT WOULD BE THAT EASY')
                            print('in a flash of static and light you find that the chair along with the individual has vanished')
                            print('But you hear breathing coming from behind you and turn to inspect...')
                            print('The individual in the chair now resides before you')
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                    else:
                        print('the loudspeaker fizzes out')
                        print('you decide to sit down a regroup')
                        for i in range(40):
                            print('.')
                        print('You look up and notice that two strips of purple light are turning on dully on the ceiling')
                        print('Upon futher evaluation you notice the walls are slightly glowing but not fully, only in some sort of distinct shape')
                        print('They seem to resemble a path with the phrase \n*Trust*\n scrawled on the wall in blood and the logo of a corporation that looks somewhat familiar')
                        print('You get up and decide to follow the path on the wall and it leads you to the stairway door')
                        print('You open the door and find not a staricase but an elongated roo, with what seems to be a figure sitting down, staring out a window')
                        print('You move closer into the room and pause for a second')
                        print('A great metal clap is heard from behind you and you find that the door that you entered through is no longer existant')
                        print('Do you approach the figure or call to them at a distance?')
                        decision = input('a or b\n')
                        if decision == 'a':
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                        else:
                            print('You take a few more steps forward and call out')
                            print('Hello? Who are you?')
                            print('The loudspeaker static is heard again')
                            print('DO YOU REALLY THINK IT WOULD BE THAT EASY')
                            print('in a flash of static and light you find that the chair along with the individual has vanished')
                            print('But you hear breathing coming from behind you and turn to inspect...')
                            print('The individual in the chair now resides before you')
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
            else:
                print('You start to venture down the other hallway')
                print('The speaker sizzles to life')
                print('TRUST THE PROCESS...FOLLOW THE PATH')
                print('and the static is heard again...')
                print('you start to tread down the correct hallway, firmly grasping the bat')
                print('as you continue down the hall, you hear a buzz on what seems to be a loudspeaker followed by a slight noise rather similar to breathing, followed by the same silence that accompanied you before')
                print('you hear a sharp fizzing noise followed by a pop')
                print('The hallway lights fizz out and you are left to the pitch black of before')
                print('Do you pull out your lighter or do you trust you remember where the stair door was')
                decision = input('a or b\n')
                if decision == 'a':
                    print('You reach for your lighter but feel nothing left in your pocket but the scrap of paper with\n*Trust the process*\nscrawled on it')
                    print('The all but familiar static of the loud speaker is heard again')
                    print('The voice speaks again saying in a rather raspy older bodied voice \n"Ahhhhh Do you not Trust %s?"' % name)
                    print('You stand in the darkness trembling, feeling light headed, with pain bolting down your leg with every step you do and dont take')
                    print('Do you respond to the voice?')
                    decsion = input('a or b\n')
                    if decision == 'a':
                        print('What do you say?')
                        speech = input()
                        print('%s...interesting...you must trust %s, its the only way' % (speech, name))
                        print('you scream "WHAT DO YOU MEAN???"')
                        print('the loudspeaker fizzes out and the voice never responds')
                        print('you decide to sit down a regroup')
                        for i in range(40):
                            print('.')
                        print('You look up and notice that two strips of purple light are turning on dully on the ceiling')
                        print('Upon futher evaluation you notice the walls are slightly glowing but not fully, only in some sort of distinct shape')
                        print('They seem to resemble a path with the phrase \n*Trust*\n scrawled on the wall in blood and the logo of a corporation that looks somewhat familiar')
                        print('You get up and decide to follow the path on the wall and it leads you to the stairway door')
                        print('You open the door and find not a staricase but an elongated roo, with what seems to be a figure sitting down, staring out a window')
                        print('You move closer into the room and pause for a second')
                        print('A great metal clap is heard from behind you and you find that the door that you entered through is no longer existant')
                        print('Do you approach the figure or call to them at a distance?')
                        decision = input('a or b\n')
                        if decision == 'a':
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                        else:
                            print('You take a few more steps forward and call out')
                            print('Hello? Who are you?')
                            print('The loudspeaker static is heard again')
                            print('DO YOU REALLY THINK IT WOULD BE THAT EASY')
                            print('in a flash of static and light you find that the chair along with the individual has vanished')
                            print('But you hear breathing coming from behind you and turn to inspect...')
                            print('The individual in the chair now resides before you')
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raiimport math
def printBoard(char):
    print('Here is the board:')
    for i in range(len(char)):
        print('%2s |' % char[i], end = ' ')
        if (i + 1) % (math.sqrt(len(char))) == 0:
            print()

def fullRow(char, spec_char, length):
    my_list = []
    char_list = []
    for i in range(int(length)):
        char_list.append(spec_char)
    for i in range(len(char)):
        my_list.append(char[i])
        if (i + 1) % (math.sqrt(len(char))) == 0:
            if char_list == my_list:
                return True
        if len(my_list) == int(length):
            my_list = []
    return False

def fullCol(char, spec_char, length):
    char_list = []
    for i in range(int(length)):
        char_list.append(spec_char)
    for j in range(int(length)):
        my_list = []
        for i in range(j, len(char), int(length)):
            if char[i] == spec_char:
                my_list.append(char[i])
            if my_list == char_list:
                return True
    return False

def fullDiag(char, spec_char, length):
    char_list = []
    my_list = []
    for i in range(int(length)):
        char_list.append(spec_char)
    for i in range(0, len(char), (int(length) + 1)):
        if char[i] == spec_char:
            my_list.append(char[i])
        if my_list == char_list:
            return True
    my_list = []
    for i in range((int(length) - 1), len(char), (int(length) - 1)):
        if char[i] == spec_char:
            my_list.append(char[i])
        if my_list == char_list:
            return True
    return False

# END OF FUNCTION DEFINITIONS

# MAIN PART OF THE PROGRAM
def main():
    # YOUR CODE BEGINS HERE
    con = False
    tmp = input()
    board = tmp.split(',')
    if round(math.sqrt(len(board))) == int(math.sqrt(len(board))):
        for i in range(len(board)):
            board[i] = (i + 1)
        while con == False:
            printBoard(board)
            user_input = int(input(('Enter move for x (1-%d): ' % (len(board)))))
            print(user_input)

            board[user_input - 1] = 'X'
            if (fullRow(board, 'X', math.sqrt(len(board))) or fullCol(board, 'X', math.sqrt(len(board))) or\
            fullDiag(board, 'X', math.sqrt(len(board)))) == True:
                winner = 'X'
                break

            if (board[user_input - 2] != 'X') and (board[user_input - 2] != 'O') and (user_input > 1):
                board[user_input - 2] = 'O'
            else:
                board[user_input] = 'O'
            if (fullRow(board, 'O', math.sqrt(len(board))) or fullCol(board, 'O', math.sqrt(len(board))) or\
            fullDiag(board, 'O', math.sqrt(len(board)))) == True:
                winner = 'O'
                break

        printBoard(board)
        print('The winner is %s !' % winner)
    else:
        print('The board must be a square!')

if __name__ == "__main__":
    main()se the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                    else:
                        print('the loudspeaker fizzes out')
                        print('you decide to sit down a regroup')
                        for i in range(40):
                            print('.')
                        print('You look up and notice that two strips of purple light are turning on dully on the ceiling')
                        print('Upon futher evaluation you notice the walls are slightly glowing but not fully, only in some sort of distinct shape')
                        print('They seem to resemble a path with the phrase \n*Trust*\n scrawled on the wall in blood and the logo of a corporation that looks somewhat familiar')
                        print('You get up and decide to follow the path on the wall and it leads you to the stairway door')
                        print('You open the door and find not a staricase but an elongated roo, with what seems to be a figure sitting down, staring out a window')
                        print('You move closer into the room and pause for a second')
                        print('A great metal clap is heard from behind you and you find that the door that you entered through is no longer existant')
                        print('Do you approach the figure or call to them at a distance?')
                        decision = input('a or b\n')
                        if decision == 'a':
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                        else:
                            print('You take a few more steps forward and call out')
                            print('Hello? Who are you?')
                            print('The loudspeaker static is heard again')
                            print('DO YOU REALLY THINK IT WOULD BE THAT EASY')
                            print('in a flash of static and light you find that the chair along with the individual has vanished')
                            print('But you hear breathing coming from behind you and turn to inspect...')
                            print('The individual in the chair now resides before you')
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                else:
                    print('You stumble and reach for the wall for some guidance')
                    print('The speaker static is heard again')
                    print('NOT ONLY DO WE HAVE A FIGHTER...WE HAVE AN IGNORANT, ARROGANT FIGHTER IN OUR MIDST...ILL HELP YOU OUT.....')
                    if 0 == 0:
                        print('the loudspeaker fizzes out and the voice never responds')
                        print('you decide to sit down a regroup')
                        for i in range(40):
                            print('.')
                        print('You look up and notice that two strips of purple light are turning on dully on the ceiling')
                        print('Upon futher evaluation you notice the walls are slightly glowing but not fully, only in some sort of distinct shape')
                        print('They seem to resemble a path with the phrase \n*Trust*\n scrawled on the wall in blood and the logo of a corporation that looks somewhat familiar')
                        print('You decide to follow the path on the wall and it leads you to the stairway door')
                        print('You open the door and find not a staricase but an elongated room=, with what seems to be a figure sitting down, staring out a window')
                        print('You move closer into the room and pause for a second')
                        print('A great metal clap is heard from behind you and you find that the door that you entered through is no longer existant')
                        print('Do you approach the figure or call to them at a distance?')
                        decision = input('a or b\n')
                        if decision == 'a':
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                        else:
                            print('You take a few more steps forward and call out')
                            print('Hello? Who are you?')
                            print('The loudspeaker static is heard again')
                            print('DO YOU REALLY THINK IT WOULD BE THAT EASY')
                            print('in a flash of static and light you find that the chair along with the individual has vanished')
                            print('But you hear breathing coming from behind you and turn to inspect...')
                            print('The individual in the chair now resides before you')
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                    else:
                        print('the loudspeaker fizzes out')
                        print('you decide to sit down a regroup')
                        for i in range(40):
                            print('.')
                        print('You look up and notice that two strips of purple light are turning on dully on the ceiling')
                        print('Upon futher evaluation you notice the walls are slightly glowing but not fully, only in some sort of distinct shape')
                        print('They seem to resemble a path with the phrase \n*Trust*\n scrawled on the wall in blood and the logo of a corporation that looks somewhat familiar')
                        print('You get up and decide to follow the path on the wall and it leads you to the stairway door')
                        print('You open the door and find not a staricase but an elongated roo, with what seems to be a figure sitting down, staring out a window')
                        print('You move closer into the room and pause for a second')
                        print('A great metal clap is heard from behind you and you find that the door that you entered through is no longer existant')
                        print('Do you approach the figure or call to them at a distance?')
                        decision = input('a or b\n')
                        if decision == 'a':
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                        else:
                            print('You take a few more steps forward and call out')
                            print('Hello? Who are you?')
                            print('The loudspeaker static is heard again')
                            print('DO YOU REALLY THINK IT WOULD BE THAT EASY')
                            print('in a flash of static and light you find that the chair along with the individual has vanished')
                            print('But you hear breathing coming from behind you and turn to inspect...')
                            print('The individual in the chair now resides before you')
                            print('You slowly approach the figure masked in shadow')
                            print('In the faint light coming from the window you find the figure to have a bag over head and be tied at both hand and foot')
                            print('In the corner of the room a television monitor comes to life showing a screen of static, with the static noise filling your ears')
                            print('Attached to the monitor is a note not unlike the one you found earlier in your pocket')
                            print('*Choose from the three')
                            print('You ask what the note means and the television comes into focus revealing a woman not unlike that of the man presented before you, tied and gagged')
                            print('Do you ask who the third is or choose from the two?')
                            decision = input('a or b\n')
                            if decision == 'a':
                                print('The sizzle and static of the loudspeaker can be heard yet again')
                                print('"are there not three individuals in your presence"')
                                print('You ask as to who the two indivduals are')
                                print('"The man sitting before your eyes is an indivdual that resides in deaths row"')
                                print('"The woman before you...you know her"')
                                print('You ask who the third individual is and you recieve silence in response as the loudspeaker comes to a halt, replacing the voice again with static')
                                print('Which individual do you pick?')
                                print('The man, the woman, or individual 3')
                                print('1,2,or3')
                                decision = int(input())
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat')
                                        print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                        print('But alas ... till next time %' % name)
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                else:
                                    print('The loudspeak comes to life for the last time')
                                    print('SO I AM CURIOUS ... WHO DO YOU THINK INDIVIDUAL 3 IS?')
                                    print('SEEING AS IM JUST A COMPUTER PROGRAM...')
                                    print('you step back in awe...realizing that the feeling in your gut...that you were the individual ... was true')
                                    print('IT SEEMS YOU CHOSE QUITE NICELY FOR ME ... LESS WORK')
                                    print('But alas ... till next time %' % name)

                            else:
                                print('IM ASTOUNDED BY YOUR LACK OF INTEREST IN DETAIL BUT ALAS...WHO?')
                                print('Who do you pick?')
                                decision = int(input('1 or 2\n'))
                                if decision == 1:
                                    print('Batterrrrrrrrrrr UP!')
                                    print('The loudspeaker statics out')
                                    print('You stare down at the bat and contemplate')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        for i in range(10):
                                            print('*')
                                        print('BANG')
                                        print('A gun shot rings in your ears, pausing your swing at the man')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)
                                    else:
                                        pass
                                elif decision == 2:
                                    print('WRONG!')
                                    print('The loudspeaker statics out')
                                    print('You hear a lound pop and find the figure on the TV slouched over, bleeding from their head')
                                    print('Your Turn!')
                                    print('You exclaim what do you mean?!')
                                    print('YOU DIDNT REALLY THINK IT WOULD BE THAT EASY...I TAKE ONE, YOU TAKE THE OTHER....I JUST WAS CURIOUS AS TO AN ORDER HERE...')
                                    print('DO IT!')
                                    print('Do you follow through?')
                                    decision = input('y or n\n')
                                    if decision == 'y':
                                        print('You raise the bat to the man')
                                        print('The man breathes harder and harder, gasping for as much air as he can')
                                        print('The room is silent other than that of the static of the TV')
                                        print('You glance at the TV only to find the woman slouched over, blood dripping down her temple')
                                        print('You rapidly change glance between the bat and the TV, in complete shock over what has just occured...')
                                        print('You ask the voice, ')
                                        speech = input()
                                        print('The loudspeaker comes to life')
                                        print('You dissapoint me %s' % name)
                                        print('I took you for an individual of action, a fighter, but yet you cant take the time to swing the bat at a dummy')
                                        print('Yes....you heard that right...\ngo ahead, LOOK, the man you were to kill was nothing but a manniquin with a speaker')
                                        print('So I will ask you kindly once more, take your option!')
                                        print('*you stare down at the man in the darkness, not sure if the voice is being truthful*')
                                        print('You stare to your bat')
                                        decision = input('Do you follow through? (y or n)\n')
                                        if decision == 'y':
                                            print('You raise the bat and in rapid succession you beat the head of the figure')
                                            print('You find blood to be running down the individual')
                                            print('You slow pace till you stop completely, wondering in hysteria what you had just done')
                                            print('The speaker abruptly interupts...')
                                            print('AH THERE IS THE FIGHTER')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

                                        else:
                                            print('You drop the bat and exclaim no!')
                                            print('The speaker abruptly turns on...')
                                            print('AH HAVE WE FOUND A PACIFIST IN OUR FIGHTER?')
                                            print('I SUPPOSE ILL JUST HELP YOU OUT...YOU MADE THIS DECISION A LONG TIME AGO %s' % name)
                                            print('The man in the chair starts convulsing and then falls to the ground, dead')
                                            print("Oh an by the way, that wasn't a dummy...")
                                            print('That was ... well lets just say ... someone rather important to you')
                                            print('But alas ... till next time %s' % name)

